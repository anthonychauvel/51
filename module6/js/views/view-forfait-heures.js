...
const VFH = {
  ...
  _tplExport(a) {
    ...
    <div class="m6-card"><div class="m6-card-body">${!log.length?'<div style="font-size:0.78rem;color:var(--pierre)">Aucune modification.</div>':log.map(l=>`<div class="m6-row" style="padding:5px 0;align-items:flex-start"><div><div style="font-size:0.72rem;font-weight:500">${l.action}</div><div style="font-size:0.65rem;color:var(--pierre)">${l.detail}</div></div><span style="font-size:0.6rem;color:var(--pierre);flex-shrink:0;margin-left:8px">${new Date(l.ts).toLocaleString('fr-FR',{hour:'2-digit',minute:'2-digit',day:'2-digit',month:'2-digit'})}</span></div>`).join('')}</div></div><div class="m6-alert info" style="margin-top:16px;font-size:0.72rem"><span>&#9878;&#65039;</span><div><strong>Information juridique.</strong> Outil indicatif et pedagogique &mdash; ni avis juridique ni valeur legale. Le forfait heures (hebdomadaire ou mensuel) doit etre prevu par une convention individuelle ecrite et respecter les durees maximales de travail et les repos obligatoires. Au-dela du forfait, les heures supplementaires restent dues. En cas de doute, rapproche-toi des representants du personnel, de l inspection du travail ou d un conseil juridique. Sources : Art. L3121-56 et s. du Code du travail &middot; Legifrance.</div></div>`;
  },
